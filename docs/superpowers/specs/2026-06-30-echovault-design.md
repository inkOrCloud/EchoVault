...
| Task | 测试 | 说明 |
|:---:|:---:|:---|
| PlaylistRepository | 4 | gRPC 歌单仓储（CRUD + 歌曲操作） |
| PlaylistProvider | 3 | Riverpod 歌单状态管理 |
| PlaylistSongProvider | 2 | 歌单内歌曲状态管理 |
| 歌单元数据模型 | 2 | Playlist/PlaylistSong 模型封装 |

#### Phase 9b：歌单 UI（3天）
| Task | 测试 | 说明 |
|:---:|:---:|:---|
| PlaylistListPage | 3 | 歌单列表页（创建/删除/搜索） |
| PlaylistDetailPage | 3 | 歌单详情页（歌曲列表/添加/移除） |
| PlaylistTile Widget | 2 | 歌单项组件（封面/名称/歌曲数） |
| AddSongDialog Widget | 2 | 添加歌曲对话框（搜索/选择） |

#### Phase 9c：设备管理（2天）
| Task | 测试 | 说明 |
|:---:|:---:|:---|
| DeviceRepository | 3 | gRPC 设备仓储（列表/移除/更新） |
| DeviceProvider | 2 | Riverpod 设备状态管理 |
| DeviceListPage | 2 | 设备列表页（平台图标/最后同步时间） |
| DeviceTile Widget | 1 | 设备项组件 |

#### Phase 9d：同步状态（2天）
| Task | 测试 | 说明 |
|:---:|:---:|:---|
| SyncRepository | 3 | gRPC 同步仓储（Push/Pull/Ack） |
| SyncProvider | 2 | Riverpod 同步状态管理 |
| SyncStatusPage | 2 | 同步状态页（进度/历史/手动触发） |
| SyncStatusIndicator Widget | 1 | 同步状态指示器 |
